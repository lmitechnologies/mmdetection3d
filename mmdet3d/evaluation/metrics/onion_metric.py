...
class OnionMetric(BaseMetric):
    ...
    def __init__(self,
                 ...
                 pcd_limit_range: List[float] = [-100, -100, -100, 100, 100, 100],
        ...
        super(OnionMetric, self).__init__(
        ...
        allowed_metrics = ['bbox', 'mAP', 'LET_mAP']
        self.metrics = metric if isinstance(metric, list) else [metric]
        for metric in self.metrics:
            if metric not in allowed_metrics:
                raise KeyError("metric should be one of 'bbox', "
                               f'but got {metric}.')
